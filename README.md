...
- 老板状态缓存加速：二次扫描时离线未达阈值的老板直接跳过，大幅减少 API 请求量
...
4. 加载历史缓存数据库，对每个候选公司：
   - **缓存预判**：若缓存中老板未换人且离线未达阈值，直接跳过，不请求 API
   - 检查老板最近上线时间
   - 如老板离线超过阈值，检查员工活跃情况
   - 如存在活跃员工，标记为目标公司
   - 更新老板时间戳到缓存数据库
5. 针对目标公司，获取老板的派系信息
6. 保存更新后的缓存数据库
7. 输出 Excel 格式结果，包含直达 Torn 公司页面的链接
...
### 技术细节
- **老板状态缓存数据库**：缓存所有公司老板的 `last_action` 时间戳至 `Database/` 目录，二次扫描时跳过离线未达阈值的公司，大幅减少 API 请求量
...
⚠️ **重要警告**
- 📂 请勿删除或移动 `Database/` 文件夹，否则所有历史记录（含 IsBossDead 缓存）将永久丢失
...
| v1.6 | 2026-06-02 | 行业数据 API 升级到 v2，支持分页获取全部行业公司数据 |
