...
function saveApiKey() {
  ...
  if (!apiKey) return;
  const confirmed = confirm("⚠️ 警告：API Key 将明文存储在浏览器本地存储(localStorage)中。\n\n任何能访问此浏览器的人都可以获取此 Key。是否继续？");
  if (confirmed) {
    localStorage.setItem("torn_industry_api_key", apiKey);
  }
}

function clearApiKey() {
  localStorage.removeItem("torn_industry_api_key");
...
function showAnalysis() {
  ...
  for (let s = 9; s >= 0; s--) {
    ...
    const validDaily = (thresholdCo && thresholdCo.Weekly_Income > 0) ? thresholdCo.Daily_Income : 0;
  ...
  // ========== 第二部分：晋升门槛分析 ==========
  html += `<h3 style="margin-bottom:12px;color:#52c41a;">🚀 晋升门槛预估（一次只能升/降一星）</h3>`;
  html += `<table style="margin-bottom:20px;">
    <thead>
      <tr>
